...
class MultiComparatorApp:
    ...
    def get_stock_history(self, stock_id, start_date, end_date):
        ...
        # 建立一個暫存這檔股票 actions 的獨立口袋
        if not hasattr(self, "stock_actions_cache"):
            self.stock_actions_cache = {}
            
        try:
            # 1. 抓取最純粹、未調整的歷史收盤價
            df = yf.download(stock_code, start=start_date, end=end_date, auto_adjust=False, progress=False)
            
            # 2. 獨立調閱這檔股票的配股配息天網 (Actions)
            ticker_obj = yf.Ticker(stock_code)
            try:
                actions = ticker_obj.actions
                if actions is not None and not actions.empty:
                    # 將日期轉成字串格式儲存
                    self.stock_actions_cache[stock_id] = {
                        "div": actions['Dividends'].to_dict(),
                        "split": actions['Stock Splits'].to_dict()
                    }
                    print(f"💰 【股票天網】成功扒出股票 {stock_code} 的歷史配股配息紀錄！")
            except Exception as e:
                print(f"⚠️ 股票 {stock_code} Actions 讀取失敗(可能無配股配息): {e}")
                self.stock_actions_cache[stock_id] = {"div": {}, "split": {}}
            ...
            print(f"⚠️ 股票 {stock_code} 下載崩潰: {e}")
            return {}
    ...
    def execute_core_backtest(self, target_list, is_pct_mode=False, total_principal_pct_mode=0):
        ...
        for t in target_list:
            ...
            if col_name in df_battle.columns:
                clean_code = t["code"].strip().upper()
                is_div_fund = t.get("is_dividend", False) 
                is_stock = (t["type"] == "stock") # 🕵️‍♂️ 判定是不是股票
                ...
                print(f"📡 資產【{col_name}】計價幣別: 💎 {asset_currency} 💎 | 型態: {'📈 股票' if is_stock else '📊 基金'}")
                ...
                if is_stock:
                    # 讀取股票的 actions 字典
                    actions_pack = getattr(self, "stock_actions_cache", {}).get(t["code"], {"div": {}, "split": {}})
                    div_history = actions_pack.get("div", {})
                    stock_splits = actions_pack.get("split", {})
                elif is_div_fund:
                    div_history = self.get_fund_dividend_history(clean_code)
                ...
                # 初始持有股數/單位數
                current_units = t["money"] / first_nav
                ...
                for current_date in dates_index:
                    ...
                    ts_key = pd.Timestamp(current_date.date()) # 有些 yfinance 日期自帶時區，用 Timestamp 雙重防禦
                    
                    # ─── 🚀 股票專屬：除權/股票拆分 (配股利滾利) ───
                    if is_stock:
                        split_ratio = 1.0
                        # 比對兩種常見日期 Key 格式
                        if date_str_key in stock_splits: split_ratio = stock_splits[date_str_key]
                        elif ts_key in stock_splits: split_ratio = stock_splits[ts_key]
                        
                        # 如果大於 0 且不等於 1，代表今天除權或股票拆分了！
                        if split_ratio > 0 and split_ratio != 1.0:
                            # 👑 【股數動態放大術】：持股總數瞬間倍增，完美對抗股價跳水！
                            current_units = current_units * split_ratio
                            print(f"🔥 【股票除權通知】{col_name} 於 {date_str_key} 進行配股/拆分！倍率: {split_ratio}，最新持股數: {current_units:,.2f} 股")

                    # ─── 💰 股票與基金通用：現金股利 (配息領現) ───
                    per_unit_div = 0.0
                    if date_str_key in div_history: per_unit_div = div_history[date_str_key]
                    elif ts_key in div_history: per_unit_div = div_history[ts_key]
                    
                    if per_unit_div > 0:
                        # 領取原幣現金配息 ＝ 當前總股數 * 每股配息
                        today_dividend_pure = current_units * per_unit_div
                    ...
                    # ─── 📊 計算今日大盤總現值 ───
                    # 軌道二：不含匯率原幣市值 ＝ (最新股數 * 今日原幣股價) + 累積原幣配息池
                    today_val_pure = (current_units * current_nav) + accumulated_cash_pure
                    line_money_pure.append(today_val_pure)
                    
                    # 軌道一：含匯率台幣總現值
                    if asset_currency == "TWD":
                        today_val_fx = (current_units * current_nav) + accumulated_cash_twd
                    else:
                        today_val_fx = ((current_units * current_nav) / first_fx) * current_fx + accumulated_cash_twd
